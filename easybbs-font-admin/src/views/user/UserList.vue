        ...
        <template #op="{index,row}">
          <div class="op" v-if="row.status != -1">
            <el-dropdown trigger="click">
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  <el-dropdown-item @click="updateUserStatus(row)">{{row.status == 1 ? "禁用":"启用"}}</el-dropdown-item>
                  <el-dropdown-item @click="sendMessage(row)">发送系统信息</el-dropdown-item> 
                </el-dropdown-menu>
              </template>
            </el-dropdown>
          </div>
...
const updateUserStatus = (data) => {
  ...
  proxy.Confirm(`你确定要禁用${title}用户${data.nickName}`,async () => {
    let result = await proxy.Request({
